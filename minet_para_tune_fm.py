...
for item in para_list:
    ...
    def get_concate_embed_clk(x_input_one_hot_clk, x_input_mul_hot_clk, max_n_clk, total_embed_dim):
        ...
        print("data_embed_concat:", data_embed_concat)
        data_embed_concat = tf.reshape(data_embed_concat, [-1, max_n_clk, total_embed_dim])
        return data_embed_concat

    def get_concate_embed_clk_fm(x_input_one_hot_clk, x_input_mul_hot_clk, max_n_clk, total_embed_dim):
        data_embed_one_hot = get_masked_one_hot_clk(x_input_one_hot_clk)
        data_embed_mul_hot = get_masked_mul_hot_clk(x_input_mul_hot_clk)
        data_embed_concat = tf.concat([data_embed_one_hot, data_embed_mul_hot], 2)
        print("data_embed_concat_fm:", data_embed_concat)
        data_embed_concat = tf.reshape(data_embed_concat, [-1, max_n_clk * 5, total_embed_dim])
        return data_embed_concat
    ...
    def item_level_att_1(data_embed_clk_1, data_embed_1_re, data_embed_user_re, \
        ...
        # convert to 2D
        print("data_embed_clk_1: ", data_embed_clk_1)
        data_embed_clk_1_re = tf.reshape(data_embed_clk_1, [-1, total_embed_dim_1])
        print("data_embed_clk_1_re: ", data_embed_clk_1_re)
    ...
    # factorization machine weights and biases
    glorot = np.sqrt(2.0 / (10 + 1))
    fm_proj = tf.Variable(
                        np.random.normal(loc=0, scale=glorot, size=(10, 1)),
                        dtype=np.float32)  
    fm_projbias = tf.Variable(tf.constant(0.01), dtype=np.float32)

    w_minet = tf.Variable(tf.constant(0.5), dtype=np.float32)
    w_fm = tf.Variable(tf.constant(0.5), dtype=np.float32)
    ...
    print("x_input_user_1.shape:", x_input_user_1.shape)

    print("x_input_one_hot.shape:", x_input_one_hot.shape)
    print("x_input_mul_hot.shape:", x_input_mul_hot.shape)

    print("x_input_one_hot:", x_input_one_hot)
    print("x_input_mul_hot:", x_input_mul_hot)

    print("x_input_one_hot_1.shape:", x_input_one_hot_1.shape)
    print("x_input_mul_hot_1.shape:", x_input_mul_hot_1.shape)

    print("x_input_one_hot_1:", x_input_one_hot_1)
    print("x_input_mul_hot_1:", x_input_mul_hot_1)

    print("x_input_one_hot_2.shape:", x_input_one_hot_2.shape)
    print("x_input_mul_hot_2.shape:", x_input_mul_hot_2.shape)

    print("x_input_one_hot_2:", x_input_one_hot_2)
    print("x_input_mul_hot_2:", x_input_mul_hot_2)
    ...
    print("data_embed_user_1:", data_embed_user_1)
    print("data_embed_1:", data_embed_1)
    print("data_embed_clk_1:", data_embed_clk_1)
    print("data_embed_clk_2:", data_embed_clk_2)
    ...
    print("weight_dict[0]", weight_dict_1[0])
    print("data_embed_1_final:", data_embed_1_final)

    # factorization machine
    data_embed_clk_1_fm = get_concate_embed_clk_fm(x_input_one_hot_1, x_input_mul_hot_1, max_n_clk_1, total_embed_dim_user)
    data_embed_1_fm = tf.reshape(data_embed_1, [-1, 5, total_embed_dim_user])
    data_embed_clk_2_fm = get_concate_embed_clk_fm(x_input_one_hot_2, x_input_mul_hot_2, max_n_clk_2, total_embed_dim_user)
    data_embed_user_1_fm = tf.reshape(data_embed_user_1, [-1, 1, total_embed_dim_user])
    embed_v = tf.concat([data_embed_user_1_fm, data_embed_1_fm, data_embed_clk_2_fm, data_embed_clk_1_fm], axis=1)
    print("embed_v: ", embed_v)
    
    #first_order = tf.reduce_sum(tf.multiply(embed_v, fm_weights))

    sum_square = tf.square(tf.reduce_sum(embed_v, 1))
    square_sum = tf.reduce_sum(tf.square(embed_v), 1)
    second_order = 0.5 * tf.subtract(sum_square, square_sum)
    second_order = tf.nn.dropout(second_order, 0.9)
    print("second_order: ", second_order)
    fm_y = tf.add(tf.matmul(second_order, fm_proj), fm_projbias) #+ first_order
    #fm_y = tf.reduce_sum(second_order)
    print("fm_y: ", fm_y)
    ...
    y_hat_1 = get_y_hat(data_embed_1_final, weight_dict_1, layer_dim_1) * w_minet + fm_y * w_fm
    ...
    #loss_fm = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=fm_y, labels=y_target_1))
    loss = wgt_1*loss_1 + wgt_2*loss_2
    #loss = loss_1

    ## auc w/fm 0.7818

    ## 0.8 minet 0.2 fm, source hist, target hist
    ## wgt_2   eta     auc_1   loss_1
    ## 1.0     0.02    0.7824  0.4264400005340576
